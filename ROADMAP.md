...
| 2b | **The policy covers global presets** — a global preset holds facts about a subject who may not be the user, so the roster can carry third parties' names, tax numbers and passport numbers; §1, §2, §6 and §7 all needed real edits rather than an appended bullet, and §7's is the one the feature really needs — the user may themselves be a controller for data we can never see. Written and committed, deliberately **not pushed**: the page describes a feature the extension does not have until ROADMAP row 16 ships, so the push and a re-dated effective line belong beside that release. | 🚧 | P1 | [changelog](CHANGELOG.md) 2026-09-02 |
...
| 6b | **Licence terms** — the page saying what is free, what a licence covers and what it does not, at `/terms/`. Written and committed, deliberately **not pushed**: it describes paid tiers the extension does not have until the extension repository's rows 17, 17b and 25 ship. Its wording is [`docs/copy.md`](docs/copy.md) §1 poured out. | 🚧 | P1 | [changelog](CHANGELOG.md) 2026-09-03 |
...
| 6f | **Two live pages still say Grispy is free without qualification** — the support page's "free to everyone, with no paid tier" and the landing page's "Grispy is free". Both are accurate today and become wrong the day paid tiers ship, so the correction travels with that release rather than ahead of it. | 💭 | P1 | [copy](docs/copy.md) §7 item 4 |
