...
| 9b | **The five features with no user documentation at all** — row 9 moves the only page that exists, and it covers multi-step wizards. Nothing documents the core loop (save a form under a name, keep several sets against it, switch, fill), the Settings page (inventory, delete, export, import, the audit), the passphrase (asked once, declinable, unrecoverable), repeat groups (a page showing fewer rows than were saved, and the question a save asks), or **what a fill reports** — `filled`, `not-found`, `refused` and `unverified`. The last is the urgent one and the reason this row exists: [`docs/copy.md`](docs/copy.md) §3 and §4 now advertise that Grispy "tells you what it could not confirm", ungated on 2026-09-01, and no page anywhere says what those words mean. A claim in the listing whose vocabulary is undefined is a support request, not a feature. | 💭 | P2 | [copy](docs/copy.md) §3, §4 |
