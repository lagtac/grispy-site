...
| 9f | **The guide describes one half of a choice the extension offers** — row 35 also shipped a per-row scope tick and a Settings list that removes what was learned, and the guide has neither: *Pointing at a field without saving its value* still gives the per-form outcome as the only one on a saved form, and the *Settings* control table does not carry the learned-alias **Remove**. Row 1c's unsafe direction — the software ahead of the site — on the surface the policy's row 2c disclosure describes. | 💭 | P1 | — |
