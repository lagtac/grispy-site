...
| 9 | **Guide: forms that span several pages** — `docs/guide/multi-step-forms.md` in the extension repository is written for a user rather than a developer, and no user can read it: that repository is private. **Settled 2026-09-01 — this site becomes the source, not a mirror** (see the note below). The port is this row, and it has two halves that ship together: write `/guide/multi-step-forms/` here, and delete the extension repository's copy in the same change. Until both land, that copy stays authoritative — a row marked settled is not a file that has moved, and leaving the guide unowned in the gap is the drift this decision exists to prevent. Narrowed from "User guide" on 2026-09-01: that title described a directory holding exactly one file, and the port moves one page. Row 9b is the rest. | 📋 | P2 | — |
| 9b | **The five features with no user documentation at all** — row 9 moves the only page that exists, and it covers multi-step wizards. Nothing documents the core loop (save a form under a name, keep several sets against it, switch, fill), the Settings page (inventory, delete, export, import, the audit), the passphrase (asked once, declinable, unrecoverable), repeat groups (a page showing fewer rows than were saved, and the question a save asks), or **what a fill reports**. That last is the urgent one and the reason this row exists: [`docs/copy.md`](docs/copy.md) §3 and §4 now advertise that Grispy "tells you what it could not confirm", ungated on 2026-09-01, and nothing explains the two sentences a fill actually shows — `Filled 7 of 10 fields.` and `2 could not be verified.` **Corrected 2026-09-01:** this row first named the four outcome constants `filled`, `not-found`, `refused` and `unverified` as the vocabulary to document. It was written from `src/shared/outcomes.js` rather than from the screen, and it was wrong twice — there are five constants, not four (`unchanged` was missed), and a user never sees any of them. `not-found`, `refused` and `unchanged` all fold silently into the denominator, so a user cannot tell a field Grispy could not find from one a widget refused. The page documents the two sentences and states that limit; it does not teach names the popup withholds. **Shipped 2026-09-01** as five pages under `/guide/` with an index, every quoted string taken from the extension's source rather than from memory, and each page ending with the messages it covers and the limits it knows about. | ✅ | P2 | [changelog](CHANGELOG.md) 2026-09-01 |
