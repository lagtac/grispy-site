...
| 9 | **User guide** — `docs/guide/multi-step-forms.md` in the extension repository is written for a user rather than a developer, and no user can read it: that repository is private. **Settled 2026-09-01 — this site becomes the source, not a mirror** (see the note below). The port is this row, and it has two halves that ship together: write `/guide/multi-step-forms/` here, and delete the extension repository's copy in the same change. Until both land, that copy stays authoritative — a row marked settled is not a file that has moved, and leaving the guide unowned in the gap is the drift this decision exists to prevent. | 📋 | P2 | — |
