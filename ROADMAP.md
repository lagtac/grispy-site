...
| 2c | **The policy discloses learned field names and labels** — teaching a detail to be recognised on any form records the field name and the label as that page spelled them, which §1 had said was not kept and §2's encryption bullet did not cover. Both corrected on 2026-09-09, ahead of the build, per the container rule. **The effective date did not move with them**, which §9 of the policy and this repository's own publication contract both require; corrected here. | ✅ | P1 | [changelog](CHANGELOG.md) 2026-09-09 |
...
| 6 | **Payment provider named in the policy** — §7 promises to name a provider if a licence is ever sold. **Dropped 2026-09-15** with the paid plan; §7's sentence is conditional and stays. | 💭 | — | — |
...
| 6c | **Right of withdrawal and refund policy** — an EU distance sale carries a fourteen-day withdrawal right, which is a checkout-flow requirement as much as a page. **Dropped 2026-09-15** with the paid plan: nothing is sold. | 💭 | — | [copy](docs/copy.md) §1.3 |
| 6d | **Trader identification** — a trader selling into the EU must name itself. **Dropped 2026-09-15** with the paid plan: nothing is sold. | 💭 | — | [copy](docs/copy.md) §1.3 |
| 6e | **A page carrying the price** — the tiers side by side with what each includes. **Dropped 2026-09-15** with the paid plan. | 💭 | — | [copy](docs/copy.md) §1 |
...
| 6h | **The paid licence is no longer announced** — `/terms/` said capabilities for work use were being built and would need a paid licence, and invited email about prices. The paid plan is dropped, so the page now says only that new capabilities keep arriving, and keeps its promise that nothing free today will start needing a licence. | ✅ | P1 | [changelog](CHANGELOG.md) 2026-09-15 |
...
| 9d | **The guide described a Details tab that reached details only through a set** — the extension's roadmap row 33 splits that tab into a Details view and a Sets view, renames it **Details & sets**, and adds a **New detail** button that needs no set open. Three passages went stale at once: the *Settings* page's tab index and its Rename row, the *saved details* page's "open a set first" path, and the locked-store sentence. Corrected ahead of the build, per the container `CLAUDE.md` — the page changes first — so the guide is briefly ahead of what ships. | ✅ | P1 | [changelog](CHANGELOG.md) 2026-09-09 |
| 9e | **The guide said teaching a field needs a form Grispy has saved** — the extension's roadmap row 35 made that false: on an unsaved page the choice collapses to its "anywhere" half instead of being withdrawn. The paragraph and the message-table row that quoted the withdrawn sentence are both corrected, and the new string matches the extension's `#capture-teach-note` exactly. | ✅ | P2 | [changelog](CHANGELOG.md) 2026-09-09 |
| 9f | **The guide describes one half of a choice the extension offers** — row 35 also shipped a per-row scope tick and a Settings list that removes what was learned, and the guide has neither: *Pointing at a field without saving its value* still gives the per-form outcome as the only one on a saved form, and the *Settings* control table does not carry the learned-alias **Remove**. Row 1c's unsafe direction — the software ahead of the site — on the surface the policy's row 2c disclosure describes. **Withdrawn 2026-09-22** with row 10: its subject, the learned-alias scope tick, is removed from the extension. | 💭 | — | — |
| 10 | **The site stops describing sets of details** — extension roadmap row 42 removes cross-form filling. The policy, the guide, the landing page and the listing copy lose every passage about it, `guide/saved-details/` is deleted, and the popup's *This form* help sentence moves to *saving and filling*. | ✅ | P1 | [changelog](CHANGELOG.md) 2026-09-22 |
