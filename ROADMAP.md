...
| 9 | **User guide** — `docs/guide/multi-step-forms.md` in the extension repository is written for a user rather than a developer, and no user can read it: that repository is private. **Settled 2026-09-01 — this site becomes the source, not a mirror** (see the note below). What remains is the port: one page per guide topic under `/guide/`, starting with the multi-step one that already exists in prose. | 📋 | P2 | — |
