...
| 2b | **The policy covers global presets** — a global preset holds facts about a subject who may not be the user, so the roster can carry third parties' names, tax numbers and passport numbers; §1, §2, §6 and §7 all needed real edits rather than an appended bullet, and §7's is the one the feature really needs — the user may themselves be a controller for data we can never see. This row said the change was committed and deliberately **not pushed** until ROADMAP row 16 ships. **That is wrong, and was wrong when written: it was pushed on 2026-09-02 in `8e5104c` and has been live since.** So the served policy describes a feature no released build has — the same defect row 6g corrects on `/terms/`, and milder only because a policy that over-describes what is stored errs safe. Row 16 is what closes it. | 🚧 | P1 | [changelog](CHANGELOG.md) 2026-09-02 |
